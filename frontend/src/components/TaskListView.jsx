import { cardStyle, color, eyebrow, font, pageTitle, radius, shadow } from '../theme.js';
import { STATUS_LABEL, dayDiff, sortTasks, today } from '../utils.js';
...
export default function TaskListView({ user, tasks, filter, onFilter, onOpen, onToggle, onCreate }) {
  ...
  const list = sortTasks(filter === 'all' ? tasks : tasks.filter((t) => t.status === filter));
  ...
  return (
    <div style={{ animation: 'rise .34s ease both' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7, margin: '30px 0 16px' }}>
